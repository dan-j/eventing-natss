...
func buildStreamConfig(streamName, subject string, config *v1alpha1.StreamConfig) jetstream.StreamConfig {
	if config == nil {
	...
	return jetstream.StreamConfig{
		Name:         streamName,
		Subjects:     subjects,
		Retention:    utils.ConvertRetentionPolicy(config.Retention, jetstream.LimitsPolicy),
		MaxConsumers: config.MaxConsumers,
		MaxMsgs:      config.MaxMsgs,
		MaxBytes:     config.MaxBytes,
		Discard:      utils.ConvertDiscardPolicy(config.Discard, jetstream.DiscardOld),
		MaxAge:       config.MaxAge.Duration,
		MaxMsgSize:   config.MaxMsgSize,
		Storage:      utils.ConvertStorage(config.Storage, jetstream.FileStorage),
...
func buildConsumerConfig(consumerName string, template *v1alpha1.ConsumerConfigTemplate, retryConfig *kncloudevents.RetryConfig) jetstream.ConsumerConfig {
	...
	if template != nil {
		consumerConfig.DeliverPolicy = utils.ConvertDeliverPolicy(template.DeliverPolicy, jetstream.DeliverAllPolicy)
		...
		consumerConfig.ReplayPolicy = utils.ConvertReplayPolicy(template.ReplayPolicy, jetstream.ReplayInstantPolicy)
